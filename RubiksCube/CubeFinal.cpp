// Name: Tung Thanh Dao
// E-mail: thdx3393@hochschule-trier.de
// Hausarbeit WS22/23
...
void CubeFinal::Render(float aspectRatio)
{
	...
	float offset = m_cubeRenderer.GetCubeletExtension();
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			for (int k = 0; k < 3; k++)
			{
				...
				m_cubeRenderer.Render(compoundTransformation);
			}
		}
	}
}
